...
def crack_refine_new(l, conv_size=(3, 3, 7, 7, 11, 11), phase_train=True):
    ...
    l = conv_layer_with_bn(l, [1, 1, l.get_shape().as_list()[3], 32], phase_train, True, name='mapping')
...
def inference(images, labels, batch_size, phase_train):
    ...
    with tf.variable_scope('dense_block_1'):
        ...
        dense1 = dense_block(dense1, layers=4, growth_rate=16, phase_train=phase_train)
        dense1 = conv_layer_with_bn(dense1, [1, 1, dense1.get_shape().as_list()[3], 64], phase_train, True,
                                    name='transition')
        # pool2
    ...
    with tf.variable_scope('dense_block_2'):
        ...
        dense2 = dense_block(dense2, layers=4, growth_rate=16, phase_train=phase_train)
    ...
    with tf.variable_scope('dense_block_3'):
        ...
        dense3 = dense_block(dense3, layers=4, growth_rate=16, phase_train=phase_train)
    ...
    with tf.variable_scope('atrous_SPP'):
        ...
        pool4 = conv_layer_with_bn(pool4, [1, 1, pool4.get_shape().as_list()[3], 256], phase_train, True,
                                   name='trainsition')
    ...
    with tf.variable_scope('decode_4'):
        ...
        conv_decode4 = conv_layer_with_bn(concat4, [3, 3, concat4.get_shape().as_list()[3], 128], phase_train, False,
                                          name="conv_decode4")
    ...
    with tf.variable_scope('decode_3'):
        ...
        conv_decode3 = conv_layer_with_bn(concat3, [3, 3, concat3.get_shape().as_list()[3], 64], phase_train, False,
                                          name="conv_decode3")
...
def test(FLAGS):
    ...
    with tf.Session(config=config) as sess:
        # Load checkpoint
        ckpt = "D:/yel/goaf/lab/multi_input_dense_maxv2/model.ckpt-35000"
        if ckpt:
        ...
        for image_batch, label_batch in zip(images, labels):
            ...
            # writeImage(im[0], 'D:/yel/goaf/test_image/multi_input/'+str(image_filenames[count]).split('\\')[-1])
