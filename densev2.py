...
tf.app.flags.DEFINE_integer('max_steps', "5000", """ max_steps """)
...
def batch_norm(x_tensor, name=None):
    ...
    L = tf.nn.batch_normalization(x_tensor, mean, variance, 0.01, 1, 1e-5, name=name)
    return L
...
def conv_concate(x, concate_layer, growth_rate, name):
    ...
    with tf.variable_scope(name):
        if not concate_layer:
            l = conv2d(x, conv_num_outputs=growth_rate, conv_ksize=(7, 7), conv_strides=(1, 1), name='bn_relu_conv')
        else:
            l = tf.concat(concate_layer, axis=3)
            l = tf.concat([x, l], axis=3)
            l = conv2d(l, conv_num_outputs=growth_rate, conv_ksize=(7, 7), conv_strides=(1, 1), name='bn_relu_conv')
    return l
...
def dense_atrous_block(l, layers=3, growth_rate=12, dilate_rate=(2,4,6)):
    a = []
    a.append(l)
    for i in enumerate(dilate_rate):
...
def atrous_SPP(l, dilate_rate=(6, 12, 18)):
    ...
    L1 = conv2d_nobn(l, 64, (1, 1), (1, 1), name='conv_1x1')
    L2 = atrous_conv2d_nobn(l, 64, (3, 3), rate=6, name='atrous_6')
    L3 = atrous_conv2d_nobn(l, 64, (3, 3), rate=12, name='atrous_12')
    L4 = atrous_conv2d_nobn(l, 64, (3, 3), rate=18, name='atrous_18')
...
def deconv_layer(inputT, f_shape, output_shape, stride=2, name=None):
    ...
    with tf.variable_scope(name):
        weights = get_deconv_filter(f_shape)  # 反卷积中的权重为[h,w,output_channels,input_channels]
...
def transition(l, name=None):
    with tf.variable_scope(name):
        l = conv2d(l, 64, (1, 1), (1, 1), name='conv_1x1')
        l = avgpool(l, (2, 2), (2, 2))
    return l
...
def inference(x, labels, grwoth_rate=12, training=None):
    ...
    l = tf.nn.lrn(x, depth_radius=5, bias=1.0, alpha=0.0001, beta=0.75,  # local response normalization.
                  name='norm1')
    l = conv2d_nobn(l, 16, (5, 5), (2, 2), 'conv0')  # l:256*256
    # Encoder process
    with tf.variable_scope('dense_block1'):
        l = dense_block(l, layers=4)
        end_points['dense_block1'] = l
        l = transition(l, name='transition')  # l:128*128
    with tf.variable_scope('dense_block2'):
        l = dense_block(l, layers=3)
        end_points['dense_block2'] = l
        l = transition(l, name='transition')  # l:64*64
    with tf.variable_scope('dense_block3'):
        l = dense_block(l,layers=2)
        end_points['dense_block3'] = l
        l = transition(l,name='transition')   # l:32*32
    with tf.variable_scope('aspp'):
        l = atrous_SPP(l)
        l = conv2d(l, 64, (1, 1), (1, 1), name='transition')

    # Decoder process
    with tf.variable_scope('decoder_1'):
        l = deconv_layer(l, [2, 2, 64, 64], [BATCH_SIZE, 64, 64, 64], 2, name='deconv') # 128x128
        l = tf.concat([l,end_points['dense_block3']],axis=3)
        l = conv2d(l, 64, (7, 7), (1, 1), name='conv_7x7')
    with tf.variable_scope('decoder_2'):
        l = deconv_layer(l, [2, 2, 64, 64], [BATCH_SIZE, 128, 128, 64], 2, name='deconv')   #256x256
        l = tf.concat([l, end_points['dense_block2']], axis=3)
        l = conv2d(l, 64, (7, 7), (1, 1), name='conv_7x7')
    with tf.variable_scope('decoder_3'):
        l = deconv_layer(l, [2, 2, 64, 64], [BATCH_SIZE, 256, 256, 64], 2, name='deconv')   #512x512
        l = tf.concat([l, end_points['dense_block1']], axis=3)
        l = conv2d(l, 64, (7, 7), (1, 1), name='conv_7x7')
    with tf.variable_scope('decoder_4'):
        l = deconv_layer(l, [2, 2, 64, 64], [BATCH_SIZE, 512, 512, 64], 2, name='deconv')
        l = conv2d(l, 64, (7, 7), (1, 1), name='conv_7x7')
    with tf.variable_scope('output'):
        l = output(l, 2, (1, 1), (1, 1), name='output')
...
def training(is_finetune=False):
    ...
    with tf.Graph().as_default():
        ...
        eval_prediction, loss = inference(train_data_node, train_labels_node,phase_train)
