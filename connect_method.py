...
def get_contours_dist(points, cnt_left):
    """计算当前轮廓上的点到其他轮廓之间的距离"""
    distance = np.ones((len(cnt_left), len(points)))
    for id, point in enumerate(points):
        for j in range(len(cnt_left)):
            dist_tmp = []
            for cnt_point in cnt_left[j]:
                cnt_point = np.squeeze(cnt_point)
                dist = np.linalg.norm(cnt_point - point)
                dist_tmp += [dist]

            distance[j][id] = np.min(dist_tmp)  # 点到单个轮廓的距离
    distance = np.min(distance, axis=1)
    return distance


def connect_cntv2(img, contours, area, dist_cond=8, angle_cond=20):
    ...
    for i in range(len(rect)):
        ...
        dist_cnt = get_contours_dist(points, contours[i + 1:])
        for j in range(len(dist)):
            # 满足条件的轮廓即为我们要进行连接的轮廓
            if dist[j] < dist_cond and angle_diff[j] < angle_cond and dist_cnt[j] < 70 and area[i+j+1] >10:
                wide = math.floor(np.minimum(sizes[i][0], sizes[i][1]) / 2)
                if wide > 12:
                    wide = 8
                cnt_left = contours[i + j + 1]
                ...
                point = points[arg]
                point_left = get_point_with_small_dist(cnt_left, point)  # 找到待连接轮廓上距当前轮廓最近的点
                cv2.line(img, tuple(point), tuple(point_left), color=(int(np.max(img))), thickness=8)
                # 连接轮廓之后重新寻找轮廓再次连接
                # cv2.imshow("img", img)
                # cv2.waitKey()
                return img, terminate
            # elif dist[j] < dist_cond and dist_cnt[j] < 30:
            #     points = get_contour_point(contours[i], (x, y, k))  # 计算当前轮廓上距离走向直线最近的点
            #     wide = math.floor(np.minimum(sizes[i][0], sizes[i][1]) / 2)
            #     if wide > 12:
            #         wide = 10
...
def connect_contour(img, dist_cond=10, angle_cond=10):
    contours, _,area = findContours(img,get_area=True)
...
def connect_contourv2(img, small_cond=10, dist_cond=10, angle_cond=10):
    contours, _ = findContours(img)
    img, contours, area = remove_small_region(img, contours,small_region=small_cond)
    if len(contours)<2:
        return img
    img, term = connect_cntv2(img, contours, area,dist_cond=dist_cond,angle_cond=angle_cond)
    while True:
        contours, _ = findContours(img)
        img, contours, area = remove_small_region(img, contours, small_region=small_cond)
        if len(contours)<2:
...
def test_run():
    ...
    img_copy = np.array(img_cv)
    img_copy = np.where(img_copy == 1, 255 * img_cv, img_cv)
    # contours, _ = cv2.findContours(img_cv, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    img = connect_contourv2(img_copy)
...
def post(im):
    img_copy = np.array(im)
    img_copy = np.where(img_copy == 1, 255 * img_copy, img_copy)
    if not isinstance(img_copy, np.uint8):
        img_copy = img_copy.astype(np.uint8)
    img_copy = connect_contourv2(img_copy,small_cond=1,dist_cond=5,angle_cond=10)
    img = np.where(img_copy == 255, img_copy / 255, img_copy)
