...
def inference(images, labels, batch_size, phase_train):
    ...
    with tf.variable_scope('dense_block_1'):
        ...
        dense1 = dense_block(pool1,layers=4,growth_rate=24,phase_train=phase_train)
...
def test(FLAGS):
    ...
    with tf.Session(config=config) as sess:
        # Load checkpoint
        ckpt = "D:/yel/goaf/lab/single_input_dense_with_max_detail/model.ckpt-35000"
        if ckpt:
        ...
        for image_batch, label_batch in zip(images, labels):
            ...
            if (FLAGS.save_image):
                ...
                writeImage(im[0], 'D:/yel/goaf/test_image/single_input/patch'+str(image_filenames[count]).split('\\')[-1])
