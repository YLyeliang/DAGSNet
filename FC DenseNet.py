...
tf.app.flags.DEFINE_string('image_dir', "D:/yel/refine/512x512/train.txt", """ path to CamVid image """)
tf.app.flags.DEFINE_string('test_dir', "D:/yel/refine/512x512/test.txt", """ path to CamVid test image """)
tf.app.flags.DEFINE_string('val_dir', "D:/yel/refine/512x512/val.txt", """ path to CamVid val image """)
tf.app.flags.DEFINE_integer('max_steps', "30000", """ max_steps """)
...
NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = 1700
...
def conv_layer_with_bn(inputT, shape, train_phase, activation=True, name=None):
    ...
    with tf.variable_scope(name) as scope:
        kernel = _variable_with_weight_decay('ort_weights', shape=shape, initializer=tf.random_uniform_initializer(),
                                             wd=None)
        conv = tf.nn.conv2d(inputT, kernel, [1, 1, 1, 1], padding='SAME')
        biases = _variable_on_cpu('biases', [out_channel], tf.constant_initializer(0.0))
        bias = tf.nn.bias_add(conv, biases)
        bias = tf.nn.dropout(bias, 0.2)
...
def deconv_layer(inputT, f_shape, output_shape, stride=2, name=None):
    ...
    with tf.variable_scope(name):
        weights = tf.get_variable('up_filter', initializer=tf.random_uniform_initializer(), shape=f_shape)
...
def conv_concate(x, growth_rate, conv_size, phase_train, name):
    shape = x.get_shape().as_list()
    with tf.variable_scope(name):
        l = conv_layer_with_bn(x, [conv_size[0], conv_size[1], shape[3], growth_rate], phase_train, True, name=name)
        l = tf.concat([l, x], 3)
    return l


def dense_block(l, layers=12, growth_rate=12, conv_size=(3, 3), phase_train=True):
    for i in range(layers):
        l = conv_concate(l, growth_rate=growth_rate, conv_size=conv_size, phase_train=phase_train,
...
def inference(images, labels, batch_size, phase_train):
    ...
    norm1 = conv_layer_with_bn(images, [3, 3, 3, 48], phase_train, name='conv_1x1')  # 48
    # dense_block1
    with tf.variable_scope('dense_block_1'):
        conv1 = dense_block(norm1, layers=4, growth_rate=16, phase_train=phase_train)  # 256x256x112
        td1 = conv_layer_with_bn(conv1, [1, 1, conv1.get_shape().as_list()[3], 112], phase_train, True,
    ...
    with tf.variable_scope('dense_block_2'):
        conv2 = dense_block(pool1, layers=5, growth_rate=16, phase_train=phase_train)  # 128x128x192
        td2 = conv_layer_with_bn(conv2, [1, 1, conv2.get_shape().as_list()[3], 192], phase_train, True,
                                 name='transition')  # trainsition
        # pool2
    ...
    with tf.variable_scope('dense_block_3'):
        conv3 = dense_block(pool2, layers=7, growth_rate=16, conv_size=(3, 3), phase_train=phase_train)  # 64x64x304
        td3 = conv_layer_with_bn(conv3, [1, 1, conv3.get_shape().as_list()[3], 304], phase_train, True,
                                 name='transition')  # trainsition
        # pool3
    ...
    with tf.variable_scope('dense_block_4'):
        conv4 = dense_block(pool3, layers=10, growth_rate=16, conv_size=(3, 3), phase_train=phase_train)  # 32x32x464
        td4 = conv_layer_with_bn(conv4, [1, 1, conv4.get_shape().as_list()[3], 464], phase_train, True,
                                 name='transition')  # trainsition
        # pool4
        pool4, pool4_indices = tf.nn.max_pool_with_argmax(td4, ksize=[1, 2, 2, 1],
                                                          strides=[1, 2, 2, 1], padding='SAME', name='pool4')
    with tf.variable_scope('dense_block_5'):
        conv5 = dense_block(pool4, layers=12, growth_rate=16, conv_size=(3, 3), phase_train=phase_train)  # 16x16x656
        td5 = conv_layer_with_bn(conv5, [1, 1, conv5.get_shape().as_list()[3], 656], phase_train, True,
                                 name='transition')
        # pool5
        pool5 = tf.nn.max_pool(td5, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool5')

    with tf.variable_scope('dense_block_6'):
        conv6 = dense_block(pool5, layers=15, growth_rate=16, conv_size=(3, 3), phase_train=phase_train)  # 8x8x896
    ...
    upsample5 = deconv_layer(conv6, [3, 3, 240, 896], [batch_size, 16, 16, 240], 2, "up5")
    # decode 5
    with tf.variable_scope('decode_5'):
        concat5 = tf.concat([upsample5, td5], axis=3)
        conv_decode5 = dense_block(concat5, layers=12, growth_rate=16, conv_size=(3, 3), phase_train=phase_train)

    # upsample 4
    # upsample3 = upsample_with_pool_indices(conv_decode4, pool3_indices, conv_decode4.get_shape(), scale=2, name='upsample3')
    upsample4 = deconv_layer(conv_decode5, [3, 3, 192, 1088], [batch_size, 32, 32, 192], 2, "up4")
    # decode 4
    with tf.variable_scope('decode_4'):
        concat4 = tf.concat([upsample4, td4], axis=3)
        conv_decode4 = dense_block(concat4, layers=10, growth_rate=16, conv_size=(3, 3), phase_train=phase_train)

    # upsample3
    # upsample2 = upsample_with_pool_indices(conv_decode3, pool2_indices, conv_decode3.get_shape(), scale=2, name='upsample2')
    upsample3 = deconv_layer(conv_decode4, [3, 3, 160, 816], [batch_size, 64, 64, 160], 2, "up3")
    # decode 3
    with tf.variable_scope('decode_3'):
        concat3 = tf.concat([upsample3, td3], axis=3)
        conv_decode3 = dense_block(concat3, layers=7, growth_rate=16, conv_size=(3, 3), phase_train=phase_train)

    # upsample2
    # upsample1 = upsample_with_pool_indices(conv_decode2, pool1_indices, conv_decode2.get_shape(), scale=2, name='upsample1')
    upsample2 = deconv_layer(conv_decode3, [3, 3, 112, 576], [batch_size, 128, 128, 112], 2, "up2")
    # decode 2
    with tf.variable_scope('decode_2'):
        concat2 = tf.concat([upsample2, td2], axis=3)
        conv_decode2 = dense_block(concat2, layers=5, growth_rate=16, conv_size=(3, 3), phase_train=phase_train)
    # upsample1
    upsample1 = deconv_layer(conv_decode2, [3, 3, 80, 384], [batch_size, 256, 256, 80], 2, "up1")
    # decode 1
    with tf.variable_scope('decode_1'):
        concat1 = tf.concat([upsample1, td1], axis=3)
        conv_decode1 = dense_block(concat1, layers=4, growth_rate=16, conv_size=(3, 3), phase_train=phase_train)
    """ end of Decode """
    """ Start Classify """
    ...
    with tf.variable_scope('conv_classifier') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             ...
                                             initializer=msra_initializer(1, 256),
                                             wd=0.0005)
...
def test(FLAGS):
    ...
    with tf.Session(config=config) as sess:
        # Load checkpoint
        ckpt = "D:/tmp\goaf/log/model.ckpt-16000"
        if ckpt:
