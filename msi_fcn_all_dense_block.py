...
import math
from connect_method import post
...
tf.app.flags.DEFINE_string('finetune', '',
                           """ finetune checkpoint file """)  # D:/tmp\segnet/finetune
tf.app.flags.DEFINE_integer('batch_size', "3", """ batch_size """)
tf.app.flags.DEFINE_float('learning_rate', "1e-3", """ initial lr """)
tf.app.flags.DEFINE_string('log_dir', "D:/yel/goaf/lab/msi_fcn_all_dense_block", """ dir to store ckpt """)
...
def get_histv2(predictions, labels):
    num_class = predictions.shape[3]
    batch_size = predictions.shape[0]
    hist = np.zeros((num_class, num_class))
    for i in range(batch_size):
        img = predictions[i].argmax(2)
        if not np.max(img) ==0:
            img=post(img)
        hist += fast_hist(labels[i].flatten(), img.flatten(), num_class)
    return hist
...
def separable_conv_with_bn(inputT, shape, train_phase, activation=True, name=None):
    with tf.variable_scope(name) as scope:
        depth_filter = _variable_with_weight_decay('depth_weights', shape=shape[:-1],
                                                   initializer=orthogonal_initializer(), wd=None)
        point_filter = _variable_with_weight_decay('point_weights', shape=[1, 1, shape[2] * shape[3], shape[4]],
                                                   initializer=orthogonal_initializer(), wd=None)
        conv = tf.nn.separable_conv2d(inputT, depthwise_filter=depth_filter, pointwise_filter=point_filter,
...
def conv_concate(x, concate_layer, conv_size, growth_rate, phase_train, name):
    ...
    if not concate_layer:
        ...
        l = conv_layer_with_bn(l, [conv_size[0], conv_size[1], l.get_shape().as_list()[3], growth_rate], phase_train,
                               name=name)
    return l
...
def dense_block(l, layers=4, growth_rate=12, conv_size=(3, 3), phase_train=True):
    ...
    for i in range(layers):
        l = conv_concate(l, concate_layer=a[:i], conv_size=conv_size, growth_rate=growth_rate, phase_train=phase_train,
                         name='conv_{}.'.format(i))
        if not i == layers - 1:
            a.append(l)
        else:
...
def crack_refine(l, conv_size=(7, 7), phase_train=True):
    ...
    cr = separable_conv_with_bn(l, [conv_size[0], 1, l.get_shape().as_list()[3], 2, 32], phase_train, False,
                                name='CR_0')
    cr = separable_conv_with_bn(cr, [1, conv_size[1], cr.get_shape().as_list()[3], 2, 32], phase_train, True,
                                name='CR_1')
...
def crack_refine_new(l, conv_size=(3, 3, 7, 7, 11, 11), phase_train=True):
    # branch_1 = conv_layer_with_bn(l, [conv_size[0], 1, l.get_shape().as_list()[3],64], phase_train, False,
    #                                   name='CR_00')
    # branch_1 = conv_layer_with_bn(branch_1, [1, conv_size[1], branch_1.get_shape().as_list()[3],64],
    #                                   phase_train, True, name='CR_01')
    #
    # branch_2 = conv_layer_with_bn(l, [conv_size[2], 1, l.get_shape().as_list()[3],64], phase_train, False,
    #                                   name='CR_10')
    # branch_2 = conv_layer_with_bn(branch_2, [1, conv_size[3], branch_2.get_shape().as_list()[3],64],
    #                                   phase_train, True, name='CR_11')
    #
    # branch_3 = conv_layer_with_bn(l, [conv_size[4], 1, l.get_shape().as_list()[3],64], phase_train, False,
    #                                   name='CR_20')
    # branch_3 = conv_layer_with_bn(branch_3, [1, conv_size[5], branch_3.get_shape().as_list()[3],64],
    #                                   phase_train, True, name='CR_21')

    branch_1 = separable_conv_with_bn(l, [conv_size[0], 1, l.get_shape().as_list()[3], 2, 32], phase_train, False,
                                      name='CR_00')
    branch_1 = separable_conv_with_bn(branch_1, [1, conv_size[1], branch_1.get_shape().as_list()[3], 2, 32],
                                      phase_train, True, name='CR_01')

    branch_2 = separable_conv_with_bn(l, [conv_size[2], 1, l.get_shape().as_list()[3], 2, 32], phase_train, False,
                                      name='CR_10')
    branch_2 = separable_conv_with_bn(branch_2, [1, conv_size[3], branch_2.get_shape().as_list()[3], 2, 32],
                                      phase_train, True, name='CR_11')

    branch_3 = separable_conv_with_bn(l, [conv_size[4], 1, l.get_shape().as_list()[3], 2, 32], phase_train, False,
                                      name='CR_20')
    branch_3 = separable_conv_with_bn(branch_3, [1, conv_size[5], branch_3.get_shape().as_list()[3], 2, 32],
                                      phase_train, True, name='CR_21')
    L = tf.math.maximum(tf.math.maximum(branch_1, branch_2), branch_3)
    return L
...
def inference(images, labels, batch_size, phase_train):
    ...
    with tf.variable_scope('input_block_1'):
        ...
        conv1 = conv_layer_with_bn(conv1, [1, 1, conv1.get_shape().as_list()[3], 64], phase_train, True,
    ...
    with tf.variable_scope('dense_block_1'):
        ...
        dense1 = conv_layer_with_bn(dense1, [1, 1, dense1.get_shape().as_list()[3], 128], phase_train, True,
                                    name='transition')
        # pool2
    ...
    with tf.variable_scope('dense_block_2'):
        ...
        dense2 = conv_layer_with_bn(dense2, [1, 1, dense2.get_shape().as_list()[3], 256], phase_train, True,
                                    name='transition')
        # pool3
    ...
    with tf.variable_scope('dense_block_3'):
        ...
        dense3 = conv_layer_with_bn(dense3, [1, 1, dense3.get_shape().as_list()[3], 512], phase_train, True,
                                    name='transition')
        # pool4
        pool4 = tf.nn.max_pool(dense3, ksize=[1, 2, 2, 1],
                               strides=[1, 2, 2, 1], padding='SAME', name='pool4')  # 16x16
    with tf.variable_scope('atrous_SPP'):
        aspp = atrous_SPP(pool4, phase_train)
        aspp = conv_layer_with_bn(aspp, [1, 1, aspp.get_shape().as_list()[3], 512], phase_train, True,
                                  name='trainsition')
    ...
    upsample4 = deconv_layer(aspp, [3, 3, 512, 512], [batch_size, 32, 32, 512], 2, "up4")
    # decode 4
    with tf.variable_scope('decode_4'):
        cr4 = crack_refine_new(dense3, (1, 1, 3, 3, 5, 5), phase_train=phase_train)
        concat4 = tf.concat([upsample4, cr4], axis=3)
        conv_decode4 = dense_block(concat4, layers=4, growth_rate=24, phase_train=phase_train)
        conv_decode4 = conv_layer_with_bn(conv_decode4, [1, 1, conv_decode4.get_shape().as_list()[3], 256], phase_train, True,
                                    name='transition')

    # upsample 3
    # upsample3 = upsample_with_pool_indices(conv_decode4, pool3_indices, conv_decode4.get_shape(), scale=2, name='upsample3')
    upsample3 = deconv_layer(conv_decode4, [3, 3, 256, 256], [batch_size, 64, 64, 256], 2, "up3")
    # decode 3
    with tf.variable_scope('decode_3'):
        cr3 = crack_refine_new(dense2, (3, 3, 5, 5, 7, 7), phase_train=phase_train)
        concat3 = tf.concat([upsample3, cr3], axis=3)
        conv_decode3 = dense_block(concat3, layers=4, growth_rate=24, phase_train=phase_train)
        conv_decode3 = conv_layer_with_bn(conv_decode3, [1, 1, conv_decode3.get_shape().as_list()[3], 128], phase_train, True,
                                    name='transition')

    # upsample2
    # upsample2 = upsample_with_pool_indices(conv_decode3, pool2_indices, conv_decode3.get_shape(), scale=2, name='upsample2')
    upsample2 = deconv_layer(conv_decode3, [3, 3, 128, 128], [batch_size, 128, 128, 128], 2, "up2")
    # decode 2
    with tf.variable_scope('decode_2'):
        cr2 = crack_refine_new(dense1, (5, 5, 7, 7, 9, 9), phase_train=phase_train)
        concat2 = tf.concat([upsample2, cr2], axis=3)
        conv_decode2 = dense_block(concat2, layers=4, growth_rate=24, phase_train=phase_train)
        conv_decode2 = conv_layer_with_bn(conv_decode2, [1, 1, conv_decode2.get_shape().as_list()[3], 64], phase_train, True,
                                    name='transition')
    ...
    with tf.variable_scope('decode_1'):
        cr1 = crack_refine_new(conv1, (7, 7, 9, 9, 11, 11), phase_train=phase_train)
        concat1 = tf.concat([upsample1, cr1], axis=3)
        conv_decode1 = dense_block(concat1, layers=4, growth_rate=24, phase_train=phase_train)
        conv_decode1 = conv_layer_with_bn(conv_decode1, [1, 1, conv_decode1.get_shape().as_list()[3], 64], phase_train, True,
                                    name='transition')
    """ end of Decode """
    """ Start Classify """
...
def test(FLAGS):
    ...
    with tf.Session(config=config) as sess:
        # Load checkpoint
        ckpt = "D:/yel/goaf/lab/msi_fcn_all_dense_block/model.ckpt-48000"
        if ckpt:
        ...
        # images=[]
        # labels=[]
        # image_path = "D:\yel\detail/512x512/test/028_5.png"
        # label_path = "D:\yel\detail/512x512/testannot/028_5.png"
        # im = Image.open(image_path).resize((256, 256))
        # im = np.array(im, np.float32)
        # im = im[np.newaxis]
        # la = Image.open(label_path).resize((256, 256))
        # la = np.array(la)
        # la = la[np.newaxis]
        # la = la[..., np.newaxis]
        # images.append(im)
        ...
        iu = np.diag(hist) / (hist.sum(1) + hist.sum(0) - np.diag(hist))
        print("acc: ", acc_total)
        print("crack recall: ", np.diag(hist)[1] / hist.sum(1)[1])  # TP/TP+FP
        print("crack precision: ", np.diag(hist)[1] / hist.sum(0)[1])
        print("crack specificity: ", np.diag(hist)[0] / (np.diag(hist)[0]+hist[0][1]))  # TN/TN +FP
