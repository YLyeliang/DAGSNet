...
tf.app.flags.DEFINE_integer('batch_size', "6", """ batch_size """)
tf.app.flags.DEFINE_float('learning_rate', "1e-3", """ initial lr """)
tf.app.flags.DEFINE_string('log_dir', "D:/tmp\goaf\log", """ dir to store ckpt """)
tf.app.flags.DEFINE_string('image_dir', "D:\AerialGoaf/512x512/train.txt", """ path to CamVid image """)
tf.app.flags.DEFINE_string('test_dir', "D:\AerialGoaf/512x512/test.txt", """ path to CamVid test image """)
tf.app.flags.DEFINE_string('val_dir', "D:\AerialGoaf/512x512/val.txt", """ path to CamVid val image """)
...
def conv_concate(x, concate_layer,conv_size,multiplier ,growth_rate,phase_train, name):
    shape = x.get_shape().as_list()
    if not concate_layer:
        l = separable_conv_with_bn(x, [conv_size[0], conv_size[1], shape[3],multiplier,growth_rate], phase_train, name=name)
    else:
...
def dense_block(l,layers=4,multiplier=2 , growth_rate=12,conv_size=(7,7),phase_train=True):
    a = []
    a.append(l)
    for i in range(layers):
        l = conv_concate(l, concate_layer=a[:i],conv_size=conv_size,multiplier=multiplier ,growth_rate=growth_rate,phase_train=phase_train, name='conv_{}.'.format(i))
        if not i == layers - 1:
            a.append(l)
        else:
...
def inference(images, labels, batch_size, phase_train):
    ...
    with tf.variable_scope('dense_block_1'):
        ...
        conv1 = conv_layer_with_bn(conv1,[1,1,conv1.get_shape().as_list()[3],64],phase_train,False,name='transition')     # trainsition
    # pool1
    ...
    with tf.variable_scope('dense_block_2'):
        conv2 = dense_block(pool1,layers=4,growth_rate=24,phase_train=phase_train)        #128x128
        conv2 = conv_layer_with_bn(conv2, [1, 1, conv2.get_shape().as_list()[3], 64], phase_train, False,
    ...
    with tf.variable_scope('dense_block_3'):
        conv3 = dense_block(pool2,layers=4,growth_rate=24,conv_size=(5,5),phase_train=phase_train)        #64x64
        conv3 = conv_layer_with_bn(conv3, [1, 1, conv3.get_shape().as_list()[3], 64], phase_train, False,
                                   name='transition')  # trainsition
    # pool3
    ...
    with tf.variable_scope('dense_block_4'):
        conv4 = dense_block(pool3,layers=4,growth_rate=24,conv_size=(3,3),phase_train=phase_train)          #32x32
        conv4 = conv_layer_with_bn(conv4, [1, 1, conv4.get_shape().as_list()[3], 64], phase_train, False,
                                   name='transition')  # trainsition
    # pool4
        pool4, pool4_indices = tf.nn.max_pool_with_argmax(conv4, ksize=[1, 2, 2, 1],
                                                      strides=[1, 2, 2, 1], padding='SAME', name='pool4')   #16x16
    with tf.variable_scope('atrous_SPP'):
        SPP = atrous_SPP(pool4,phase_train)
        SPP = tf.concat([SPP,pool4],axis=3)
        pool4 = conv_layer_with_bn(SPP,[1,1,SPP.get_shape().as_list()[3],64],phase_train,False,name='trainsition')
    ...
    upsample4 = deconv_layer(pool4, [2, 2, 64, 64], [batch_size, 32, 32, 64], 2, "up4")
    # decode 4
    conv_decode4 = conv_layer_with_bn(upsample4, [3, 3, 64, 64], phase_train, False, name="conv_decode4")

    # upsample 3
    # upsample3 = upsample_with_pool_indices(conv_decode4, pool3_indices, conv_decode4.get_shape(), scale=2, name='upsample3')
    upsample3 = deconv_layer(conv_decode4, [2, 2, 64, 64], [batch_size, 64, 64, 64], 2, "up3")
    # decode 3
    conv_decode3 = conv_layer_with_bn(upsample3, [5, 5, 64, 64], phase_train, False, name="conv_decode3")
...
def test(FLAGS):
    ...
    with tf.Session(config=config) as sess:
        # Load checkpoint
        ckpt = "D:/tmp\goaf/log/model.ckpt-11000"
        if ckpt:
        ...
        for image_batch, label_batch in zip(images, labels):
            ...
            if (FLAGS.save_image):
                ...
                writeImage(im[0], 'D:/tmp\crack/val_7_256_dense_separable_block/'+str(image_filenames[count]).split('\\')[-1])
