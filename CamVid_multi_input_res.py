...
tf.app.flags.DEFINE_string('finetune', 'D:/yel/goaf/log2/model.ckpt-49999', """ finetune checkpoint file """)  # D:/tmp\segnet/finetune
...
def crack_refine(l,conv_size=(7,7),phase_train=True):
    ...
    l = conv_layer_with_bn(l,[1,1,l.get_shape().as_list()[3],64],phase_train,True,name='mapping')
    branch = conv_layer_with_bn(l,[conv_size[0],1,l.get_shape().as_list()[3],64],train_phase=phase_train,activation=True,name='refine_0')
    branch = conv_layer_with_bn(branch, [conv_size[0], 1, l.get_shape().as_list()[3],64], train_phase=phase_train,
                                activation=True, name='refine_1')
    L = l + branch
...
def atrous_SPP(l,phase_train=True):
    ...
    L2 = atrous_conv2d(l,[3,3,l.get_shape().as_list()[3],64], rate=6,train_phase=phase_train, name='atrous_6')
    L3 = atrous_conv2d(l, [3,3,l.get_shape().as_list()[3],64], rate=12,train_phase=phase_train, name='atrous_12')
    L4 = atrous_conv2d(l, [3,3,l.get_shape().as_list()[3],64], rate=18,train_phase=phase_train, name='atrous_18')
    L = tf.concat([L1, L2, L3, L4], axis=3)
    return L

def residual_block(x,conv_size,phase_train,filters,name):
    shape = x.get_shape().as_list()
    with tf.variable_scope(name):
        L = conv_layer_with_bn(x,[1,1,shape[3],filters],phase_train,name='conv1')
        L = conv_layer_with_bn(L, [conv_size[0], conv_size[1], filters, filters], phase_train, name='conv2')
        L = conv_layer_with_bn(L,[conv_size[0],conv_size[1],filters,filters*4],phase_train,name='conv3')
        L=L+x
        return L

def residual_blocks(x,layers,conv_size=(3,3),filters=16,phase_train=True):
    x = conv_layer_with_bn(x,[3,3,x.get_shape().as_list()[3],filters*4],phase_train,True,name='conv_1')
    for i in range(layers):
        L=residual_block(x,conv_size,phase_train=phase_train,filters = filters,name ='block_{}'.format(i+1))
        x=L
    return L
...
def inference(images, labels, batch_size, phase_train):
    ...
    # input_residual_blocks()_1
    with tf.variable_scope('input_block_1'):
        conv1 = residual_blocks(norm1,layers=1,filters=16, phase_train=phase_train) #
    # pool1
        pool1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                                                      padding='SAME', name='pool1') #128x128
    # input_residual_blocks()_2
    with tf.variable_scope('input_block_2'):
        conv2 = residual_blocks(norm2,layers=1, phase_train=phase_train)        #128x128

    # residual_blocks()_1
    with tf.variable_scope('residual_blocks_1'):
        dense1 = tf.concat([pool1,conv2],axis=3)
        dense1 = residual_blocks(dense1,layers=3,filters=16, phase_train=phase_train)
    # pool2
        pool2 = tf.nn.max_pool(dense1, ksize=[1, 2, 2, 1],
                                                      strides=[1, 2, 2, 1], padding='SAME', name='pool2') #64x64
    # input_residual_blocks()_3
    with tf.variable_scope('input_block_3'):
        conv3 = residual_blocks(norm3,layers=1,conv_size=(3,3),phase_train=phase_train)        #64x64

    # residual_blocks()_2
    with tf.variable_scope('residual_blocks_2'):
        dense2 = tf.concat([pool2, conv3], axis=3)
        dense2 = residual_blocks(dense2, layers=3, filters=32, phase_train=phase_train)

    # pool3
        pool3 = tf.nn.max_pool(dense2, ksize=[1, 2, 2, 1],
                                                      strides=[1, 2, 2, 1], padding='SAME', name='pool3')
    # input_residual_blocks()_4
    with tf.variable_scope('input_block_4'):
        conv4 = residual_blocks(norm4,layers=1,conv_size=(3,3),phase_train=phase_train)          #32x32

    # residual_blocks()_3
    with tf.variable_scope('residual_blocks_3'):
        dense3 = tf.concat([pool3, conv4], axis=3)
        dense3 = residual_blocks(dense3, layers=3, filters=48, phase_train=phase_train)
    # pool4
    ...
    with tf.variable_scope('decode_4'):
        ...
        conv_decode4 = conv_layer_with_bn(concat4, [3, 3, concat4.get_shape().as_list()[3], 128], phase_train, True, name="conv_decode4")
    ...
    with tf.variable_scope('decode_3'):
        ...
        conv_decode3 = conv_layer_with_bn(concat3, [3, 3, concat3.get_shape().as_list()[3], 64], phase_train, True, name="conv_decode3")
    ...
    with tf.variable_scope('decode_2'):
        ...
        conv_decode2 = conv_layer_with_bn(concat2, [3, 3, concat2.get_shape().as_list()[3], 64], phase_train, True, name="conv_decode2")
    ...
    with tf.variable_scope('decode_1'):
        ...
        conv_decode1 = conv_layer_with_bn(concat1, [3, 3, concat1.get_shape().as_list()[3], 64], phase_train, True, name="conv_decode1")
    """ end of Decode """
    """ Start Classify """
...
def test(FLAGS):
    ...
    with tf.Session(config=config) as sess:
        # Load checkpoint
        ckpt = "D:/yel/goaf/log2/model.ckpt-61000"
        if ckpt:
