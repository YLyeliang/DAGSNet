...
tf.app.flags.DEFINE_integer('batch_size', "7", """ batch_size """)
...
def inference(images, labels, batch_size, phase_train):
    ...
    with tf.variable_scope('input_block_1'):
        conv1 = dense_block(norm1,layers=2,growth_rate=16,phase_train=phase_train) #256x256
    ...
    with tf.variable_scope('input_block_2'):
        conv2 = dense_block(norm2,layers=2,growth_rate=16,phase_train=phase_train)        #128x128
    ...
    with tf.variable_scope('input_block_3'):
        conv3 = dense_block(norm3,layers=2,growth_rate=16,conv_size=(3,3),phase_train=phase_train)        #64x64
    ...
    with tf.variable_scope('input_block_4'):
        conv4 = dense_block(norm4,layers=2,growth_rate=16,conv_size=(3,3),phase_train=phase_train)          #32x32
...
def test(FLAGS):
    ...
    with tf.Session(config=config) as sess:
        # Load checkpoint
        ckpt = "D:/yel/goaf/log/model.ckpt-24000"
        if ckpt:
