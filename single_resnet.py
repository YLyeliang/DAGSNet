...
def crack_refine(l,conv_size=(7,7),phase_train=True):
    ...
    cr = separable_conv_with_bn(cr,[1,conv_size[1],cr.get_shape().as_list()[3],2,32],phase_train,False,name='CR_1')
...
def inference(images, labels, batch_size, phase_train):
    ...
    with tf.variable_scope('dense_block_1'):
        conv1 = residual_blocks(norm1,layers=4,filters=16, phase_train=phase_train) #256x256
    ...
    with tf.variable_scope('dense_block_2'):
        conv2 = residual_blocks(pool1,layers=4,filters=32, phase_train=phase_train)        #128x128
    ...
    with tf.variable_scope('dense_block_3'):
        conv3 = residual_blocks(pool2,layers=4,filters=48,phase_train=phase_train)        #64x64
    ...
    with tf.variable_scope('dense_block_4'):
        conv4 = residual_blocks(pool3,layers=4,filters=64,phase_train=phase_train)          #32x32
...
def test(FLAGS):
    ...
    with tf.Session(config=config) as sess:
        # Load checkpoint
        ckpt = "D:/yel/goaf/lab/single_input_res/model.ckpt-48000"
        if ckpt:
