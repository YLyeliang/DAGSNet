...
def inference(images, labels, batch_size, phase_train):
    ...
    with tf.variable_scope('input_block_1'):
        conv1 = residual_blocks(norm1,layers=2,filters= 16,phase_train=phase_train) #256x256
    # pool1
        pool1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                                                      padding='SAME', name='pool1') #128x128
    # input_dense_block_2
    with tf.variable_scope('input_block_2'):
        conv2 = residual_blocks(norm2,layers=2,filters=16,phase_train=phase_train)        #128x128

    # dense_block_1
    with tf.variable_scope('dense_block_1'):
        dense1 = tf.concat([pool1,conv2],axis=3)
        dense1 = residual_blocks(dense1,layers=4,filters=32,phase_train= phase_train)
    # pool2
    ...
    with tf.variable_scope('input_block_3'):
        conv3 = residual_blocks(norm3,layers=2,filters=16,phase_train=phase_train)        #64x64
    # dense_block_2
    with tf.variable_scope('dense_block_2'):
        dense2 = tf.concat([pool2, conv3], axis=3)
        dense2 = residual_blocks(dense2, layers=4,filters=48,phase_train=phase_train)
    ...
    with tf.variable_scope('input_block_4'):
        conv4 = residual_blocks(norm4,layers=4,filters=16,phase_train=phase_train)          #32x32

    # dense_block_3
    with tf.variable_scope('dense_block_3'):
        dense3 = tf.concat([pool3, conv4], axis=3)
        dense3 = residual_blocks(dense3, layers=4,filters=48, phase_train=phase_train)
    ...
    with tf.variable_scope('decode_3'):
        cr3 = crack_refine_new(dense2,(3,3,7,7),phase_train=phase_train)
        concat3 = tf.concat([upsample3, cr3], axis=3)
        conv_decode3 = conv_layer_with_bn(concat3, [3, 3, concat3.get_shape().as_list()[3], 128], phase_train, False, name="conv_decode3")
    ...
    with tf.variable_scope('decode_2'):
        cr2 = crack_refine_new(dense1,(7,7,11,11),phase_train=phase_train)
    ...
    with tf.variable_scope('decode_1'):
        cr1= crack_refine_new(conv1,(11,11,15,15),phase_train=phase_train)
