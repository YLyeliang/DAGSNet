...
tf.app.flags.DEFINE_string('finetune', 'D:/yel/goaf/log/model.ckpt-16000',
...
tf.app.flags.DEFINE_string('log_dir', "D:/yel/goaf/log", """ dir to store ckpt """)
...
tf.app.flags.DEFINE_integer('max_steps', "40000", """ max_steps """)
...
def inference(images, labels, batch_size, phase_train):
    ...
    #     aspp = conv_layer_with_bn(aspp, [1, 1, aspp.get_shape().as_list()[3], 256], phase_train, True,
    #                               name='trainsition')
    ...
    upsample4 = deconv_layer(pool4, [3, 3, 512, 512], [batch_size, 32, 32, 512], 2, "up4")
    # decode 4
    with tf.variable_scope('decode_4'):
        ...
        conv_decode4 = conv_layer_with_bn(conv_decode4, [1, 1, conv_decode4.get_shape().as_list()[3], 256], phase_train, False,
                                          name="conv_decode4")
    ...
    with tf.variable_scope('decode_3'):
        ...
        conv_decode3 = dense_block(concat3,4,24,phase_train=phase_train)
        conv_decode3 = conv_layer_with_bn(conv_decode3, [1, 1, conv_decode3.get_shape().as_list()[3], 128], phase_train, False,
                                          name="conv_decode3")
    ...
    with tf.variable_scope('decode_2'):
        ...
        conv_decode2 = dense_block(concat2,4,24,phase_train=phase_train)
        conv_decode2 = conv_layer_with_bn(conv_decode2, [1, 1, conv_decode2.get_shape().as_list()[3], 64], phase_train, False,
                                          name="conv_decode2")
    ...
    with tf.variable_scope('decode_1'):
        ...
        conv_decode1 = dense_block(concat1,4,24,phase_train=phase_train)
        conv_decode1 = conv_layer_with_bn(conv_decode1, [1, 1, conv_decode1.get_shape().as_list()[3], 64], phase_train, False,
                                          name="conv_decode1")
    """ end of Decode """
    """ Start Classify """
...
def test(FLAGS):
    ...
    with tf.Session(config=config) as sess:
        # Load checkpoint
        ckpt = "D:/yel/goaf/lab/MSI_max_full_dense/model.ckpt-37000"
        if ckpt:
        ...
        for image_batch, label_batch in zip(images, labels):
            ...
            hist += get_histv2(dense_prediction, label_batch)
            count += 1
        acc_total = np.diag(hist).sum() / hist.sum()
        iu = np.diag(hist) / (hist.sum(1) + hist.sum(0) - np.diag(hist))
        print("acc: ", acc_total)
        print("crack accuracy: ", np.diag(hist)[1] / hist.sum(1)[1])  # TP/TP+FP
