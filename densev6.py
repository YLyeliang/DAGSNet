...
tf.app.flags.DEFINE_string('image_dir', "D:/yel/512x512/train.txt", """ path to CamVid image """)
tf.app.flags.DEFINE_string('test_dir', "D:/yel/512x512/test.txt", """ path to CamVid test image """)
tf.app.flags.DEFINE_string('val_dir', "D:/yel/512x512/val.txt", """ path to CamVid val image """)
...
def dense_block(l,layers=4,growth_rate=12,conv_size=(7,7),phase_train=True):
    a = []
    a.append(l)
...
def inference(images, labels, batch_size, phase_train):
    ...
    with tf.variable_scope('atrous_SPP'):
        ...
        pool4 = conv_layer_with_bn(pool4,[1,1,pool4.get_shape().as_list()[3],64],phase_train,True,name='trainsition')
    ...
    upsample4 = deconv_layer(pool4, [3, 3, 64, 64], [batch_size, 32, 32, 64], 2, "up4")
    # decode 4
    concat4 = tf.concat([upsample4,conv4],axis=3)
    conv_decode4 = conv_layer_with_bn(concat4, [3, 3, concat4.get_shape().as_list()[3], 64], phase_train, False, name="conv_decode4")

    # upsample 3
    # upsample3 = upsample_with_pool_indices(conv_decode4, pool3_indices, conv_decode4.get_shape(), scale=2, name='upsample3')
    upsample3 = deconv_layer(conv_decode4, [3, 3, 64, 64], [batch_size, 64, 64, 64], 2, "up3")
    # decode 3
    concat3 = tf.concat([upsample3, conv3], axis=3)
    conv_decode3 = conv_layer_with_bn(concat3, [5, 5, concat3.get_shape().as_list()[3], 64], phase_train, False, name="conv_decode3")

    # upsample2
    # upsample2 = upsample_with_pool_indices(conv_decode3, pool2_indices, conv_decode3.get_shape(), scale=2, name='upsample2')
    upsample2 = deconv_layer(conv_decode3, [3, 3, 64, 64], [batch_size, 128, 128, 64], 2, "up2")
    # decode 2
    concat2 = tf.concat([upsample2, conv2], axis=3)
    conv_decode2 = conv_layer_with_bn(concat2, [7, 7, concat2.get_shape().as_list()[3], 64], phase_train, False, name="conv_decode2")

    # upsample1
    # upsample1 = upsample_with_pool_indices(conv_decode2, pool1_indices, conv_decode2.get_shape(), scale=2, name='upsample1')
    upsample1 = deconv_layer(conv_decode2, [3, 3, 64, 64], [batch_size, 256, 256, 64], 2, "up1")
    # decode4
    concat1 = tf.concat([upsample1, conv1], axis=3)
    conv_decode1 = conv_layer_with_bn(concat1, [7, 7, concat1.get_shape().as_list()[3], 64], phase_train, False, name="conv_decode1")
    """ end of Decode """
    """ Start Classify """
...
def test(FLAGS):
    ...
    with tf.Session(config=config) as sess:
        # Load checkpoint
        ckpt = "D:/yel/goaf/log/model.ckpt-17000"
        if ckpt:
        ...
        for image_batch, label_batch in zip(images, labels):
            ...
            if (FLAGS.save_image):
                ...
                writeImage(im[0], 'D:/yel\goaf/test_image/val_8_256_dense_concat/'+str(image_filenames[count]).split('\\')[-1])
