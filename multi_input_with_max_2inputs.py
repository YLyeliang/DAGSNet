...
def inference(images, labels, batch_size, phase_train):
    ...
    with tf.variable_scope('dense_block_2'):
        ...
        dense2 = dense_block(pool2, layers=4, growth_rate=24, phase_train=phase_train)
    ...
    with tf.variable_scope('dense_block_3'):
        ...
        dense3 = dense_block(pool3, layers=4, growth_rate=24, phase_train=phase_train)
    ...
    with tf.variable_scope('atrous_SPP'):
        ...
        aspp = conv_layer_with_bn(aspp,[1,1,aspp.get_shape().as_list()[3],256],phase_train,True,name='trainsition')
    ...
    upsample4 = deconv_layer(aspp, [3, 3, 256, 256], [batch_size, 32, 32, 256], 2, "up4")
    # decode 4
...
def test(FLAGS):
    ...
    with tf.Session(config=config) as sess:
        # Load checkpoint
        ckpt = "D:/yel/goaf/lab/multi_input_dense_max_2inputs/model.ckpt-36000"
        if ckpt:
        ...
        for image_batch, label_batch in zip(images, labels):
            ...
            if (FLAGS.save_image):
                ...
                writeImage(im[0], 'D:/yel/goaf/test_image/2inputs/'+str(image_filenames[count]).split('\\')[-1])
