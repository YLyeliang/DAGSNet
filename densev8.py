...
tf.app.flags.DEFINE_string('finetune', 'D:/yel/goaf/log/model.ckpt-33000',
...
def CamVid_reader(filename_queue):
    ...
    image = tf.image.random_brightness(image, max_delta=22, seed=1)
...
def crack_refine(l, conv_size=(7, 7), phase_train=True):
    ...
    cr = separable_conv_with_bn(l, [conv_size[0], 1, l.get_shape().as_list()[3], 2, 32], phase_train, False,
                                name='CR_0')
    cr = separable_conv_with_bn(cr, [1, conv_size[1], cr.get_shape().as_list()[3], 2, 32], phase_train, False,
                                name='CR_1')
...
def inference(images, labels, batch_size, phase_train):
    ...
    with tf.variable_scope('dense_block_1'):
        conv1 = dense_block(images, layers=4, growth_rate=24, phase_train=phase_train)  # 256x256
    ...
    with tf.variable_scope('dense_block_2'):
        conv2 = dense_block(pool1, layers=5, growth_rate=24, phase_train=phase_train)  # 128x128
    ...
    with tf.variable_scope('dense_block_3'):
        conv3 = dense_block(pool2, layers=7, growth_rate=24, conv_size=(3, 3), phase_train=phase_train)  # 64x64
    ...
    with tf.variable_scope('dense_block_4'):
        conv4 = dense_block(pool3, layers=9, growth_rate=24, conv_size=(3, 3), phase_train=phase_train)  # 32x32
    ...
    with tf.variable_scope('decode_4'):
        cr4 = crack_refine(conv4, (3, 3), phase_train=phase_train)
    ...
    with tf.variable_scope('decode_3'):
        cr3 = crack_refine(conv3, (5, 5), phase_train=phase_train)
    ...
    with tf.variable_scope('decode_2'):
        cr2 = crack_refine(conv2, (7, 7), phase_train=phase_train)
...
def test(FLAGS):
    ...
    with tf.Session(config=config) as sess:
        # Load checkpoint
        ckpt = "D:/yel/goaf/log/model.ckpt-61000"
        if ckpt:
        ...
        for image_batch, label_batch in zip(images, labels):
            feed_dict = {
                test_data_node: image_batch / 255,
