...
tf.app.flags.DEFINE_integer('max_steps', "45000", """ max_steps """)
...
def get_hist(predictions, labels):  #b x w x h x c
...
def residual_block(x,conv_size,phase_train,filters,name):
    ...
    with tf.variable_scope(name):
        L = conv_layer_with_bn(x,[conv_size[0],conv_size[1],shape[3],filters],phase_train,name='conv1')
...
def residual_blocks(x,layers,conv_size=(3,3),filters=16,phase_train=True):
    x = conv_layer_with_bn(x,[3,3,x.get_shape().as_list()[3],filters],phase_train,True,name='conv_1')
    for i in range(layers):
        L=residual_block(x,conv_size,phase_train=phase_train,filters = filters,name ='block_{}'.format(i+1))
        x=L
    L = conv_layer_with_bn(x,[1,1,x.get_shape().as_list()[3],filters],phase_train,True,name='mapping')
    return L
...
def inference(images, labels, batch_size, phase_train):
    ...
    with tf.variable_scope('input_block_1'):
        conv1 = residual_blocks(norm1,layers=4,filters= 32,phase_train=phase_train) #256x256
    # pool1
        pool1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                                                      padding='SAME', name='pool1') #128x128
    # input_dense_block_2
    with tf.variable_scope('input_block_2'):
        conv2 = residual_blocks(norm2,layers=4,filters=32,phase_train=phase_train)        #128x128

    # dense_block_1
    with tf.variable_scope('dense_block_1'):
        dense1 = tf.concat([pool1,conv2],axis=3)
        dense1 = residual_blocks(dense1,layers=4,filters=64,phase_train= phase_train)
    # pool2
    ...
    with tf.variable_scope('input_block_3'):
        conv3 = residual_blocks(norm3,layers=4,filters=32,phase_train=phase_train)        #64x64
    # dense_block_2
    with tf.variable_scope('dense_block_2'):
        dense2 = tf.concat([pool2, conv3], axis=3)
        dense2 = residual_blocks(dense2, layers=4,filters=128,phase_train=phase_train)
    ...
    upsample3 = deconv_layer(pool4, [3, 3, 256, 256], [batch_size, 64, 64, 256], 2, "up3")
    # decode 3
...
def test(FLAGS):
    ...
    with tf.Session(config=config) as sess:
        # Load checkpoint
        ckpt = "D:/yel/goaf/log2/model.ckpt-36000"
        if ckpt:
        ...
        iu = np.diag(hist) / (hist.sum(1) + hist.sum(0) - np.diag(hist))
        print("acc: ", acc_total)
        print("crack recall: ", np.diag(hist)[1]/hist.sum(1)[1])        #TP/TP+FN
        print('crack precision = ',np.diag(hist)[1]/hist.sum(0)[1])     #TP/TP+FP
