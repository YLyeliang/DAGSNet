...
def CamVid_reader(filename_queue):
    ...
    label = tf.image.resize_images(label, (360, 480))
...
def conv_concate(x, concate_layer, growth_rate,phase_train, name):
    shape = x.get_shape().as_list()
    with tf.variable_scope(name):
        if not concate_layer:
            l = conv_layer_with_bn(x, [7, 7, shape[3], 64], phase_train, name="conv")
        else:
            l = tf.concat(concate_layer, axis=3)
            l = tf.concat([x, l], axis=3)
            l = conv_layer_with_bn(l, [7, 7, shape[3], 64], phase_train, name="conv")
    return l


def dense_block(l,layers=4,growth_rate=12,phase_train=True):
    a = []
    a.append(l)
    for i in range(layers):
        l = conv_concate(l, concate_layer=a[:i], growth_rate=growth_rate,phase_train=phase_train, name='conv_{}.'.format(i))
        if not i == layers - 1:
            a.append(l)
        else:
...
def inference(images, labels, batch_size, phase_train):
    ...
    norm1 = tf.nn.lrn(images, depth_radius=5, bias=1.0, alpha=0.0001, beta=0.75,  # local response normalization.
                      name='norm1')
    # conv1
    conv1 = conv_layer_with_bn(norm1, [7, 7, images.get_shape().as_list()[3], 64], phase_train, name="conv1") #512x512
    # pool1
    pool1, pool1_indices = tf.nn.max_pool_with_argmax(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                                                      padding='SAME', name='pool1')
    # conv2
    conv2 = conv_layer_with_bn(pool1, [7, 7, 64, 64], phase_train, name="conv2")        #256x256

    # pool2
    pool2, pool2_indices = tf.nn.max_pool_with_argmax(conv2, ksize=[1, 2, 2, 1],
                                                      strides=[1, 2, 2, 1], padding='SAME', name='pool2')
    # conv3
    conv3 = conv_layer_with_bn(pool2, [7, 7, 64, 64], phase_train, name="conv3")        #128x128

    # pool3
    pool3, pool3_indices = tf.nn.max_pool_with_argmax(conv3, ksize=[1, 2, 2, 1],
                                                      strides=[1, 2, 2, 1], padding='SAME', name='pool3')
    # conv4
    conv4 = conv_layer_with_bn(pool3, [7, 7, 64, 64], phase_train, name="conv4")        #64x64
